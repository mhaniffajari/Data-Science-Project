...
for i in range(10000):
    ...
    cif_no = str(random.randint(100000, 999999))  # 6-digit CIF number
...
# Save to CSV
df.to_csv('sample_data_2024.csv', index=False)
print("Data generation complete. Saved as 'sample_data_2024.csv'")
engine = to_postgres(user,server,database)
Session = sessionmaker(bind=engine)
session = Session()
session.execute('''TRUNCATE TABLE public.nasabah''')
session.commit()
session.close()
df.to_sql('nasabah', to_postgres(user,server,database),if_exists='append',index=False)
print("Data Truncated and Inserted to Postgres")
